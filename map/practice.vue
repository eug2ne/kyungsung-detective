    ...
    computed: {
        data(){
            return{
                ...
                cursors: null, //키보드 키 
                ground: null,
                col_left_h_g: null, //경복궁 왼쪽 충돌
                col_mid_h: null, //경복궁 중앙 충돌
                col_mini_h: null, //젤 작은 집
                col_left_h: null, //밑에 궁 왼쪽 테두리
                col_right_h: null, //궁 오른쪽 테두리
                col_north_o: null,//위에 궁하나
                col_west_o: null,//오른쪽에 궁하나
                col_fo1: null,//숲
                col_fo2: null,
                col_fo3: null,
                col_fo4: null,
        ...
        preload: function(){
            ...
            scene.load.image('back1', 'map_png/궁정리.png');
            scene.load.image('back2', 'map_png/건물레이어(나무밑).png');
            scene.load.image('treess', 'map_png/다리자른 나무.png');
            scene.load.spritesheet('sami', '주인공걸음마확대_frame1.png', { frameWidth: 463, frameHeight: 500 });
            scene.load.spritesheet('dude', 'map_png/dude.png', { frameWidth: 32, frameHeight: 48 });
            //충돌 이미지
            scene.load.image('house1', 'map_png/궁_0000s_0004_건물6_충돌버전.png');
            scene.load.image('tree', 'map_png/궁_0004_나무한그루_충돌.png');
            scene.load.image('left_h_g', 'map_png/경복궁왼쪽충돌.png');
            scene.load.image('mid_h', 'map_png/경복궁 충돌 중앙.png');
            scene.load.image('mini_h', 'map_png/미니집.png');
            scene.load.image('left_h', 'map_png/왼쪽집.png');
            scene.load.image('right_h', 'map_png/오른쪽집.png');
            scene.load.image('north_o', 'map_png/위에 충돌(궁하나 혼자).png');
            scene.load.image('west_o', 'map_png/오른쪽에 충돌(궁하나 혼자).png');

            scene.load.image('fo1', 'map_png/오른쪽숲1.png');
            scene.load.image('fo2', 'map_png/오른쪽숲2.png');
            scene.load.image('fo3', 'map_png/오른쪽숲3.png');
            scene.load.image('fo4', 'map_png/오른쪽숲4.png');
            scene.load.image('fo5', 'map_png/오른쪽숲5.png');
            scene.load.image('fo6', 'map_png/오른쪽숲6.png');
            scene.load.image('gr1', 'map_png/궁오른쪽1.png');
            scene.load.image('gr2', 'map_png/궁오른쪽2.png');
            scene.load.image('gr3', 'map_png/궁오른쪽3.png');
            scene.load.image('gr4', 'map_png/궁오른쪽4.png');

        },
        create: function(){
            let scene = this.game.scene.scenes[0];
            scene.add.image(700,495.25,'back1').setScale(0.5);
            platforms = scene.physics.add.staticGroup(); //그룹으로 묶는다. 
            house_m = scene.physics.add.staticGroup();
            trees = scene.physics.add.staticGroup();
            tree_sss = scene.physics.add.staticGroup();
            col_left_h_g =scene.physics.add.staticGroup();
            col_mid_h = scene.physics.add.staticGroup();
            col_west_o = scene.physics.add.staticGroup();
            col_north_o = scene.physics.add.staticGroup();
            col_mini_h = scene.physics.add.staticGroup();
            col_left_h = scene.physics.add.staticGroup();
            col_right_h = scene.physics.add.staticGroup();
            col_fo1= scene.physics.add.staticGroup();
            col_fo2 = scene.physics.add.staticGroup();
            col_fo3 = scene.physics.add.staticGroup();
            col_fo4 = scene.physics.add.staticGroup();
            col_fo5 = scene.physics.add.staticGroup();
            col_fo6 = scene.physics.add.staticGroup();
            col_gr1 = scene.physics.add.staticGroup();
            col_gr2 = scene.physics.add.staticGroup();
            col_gr3 = scene.physics.add.staticGroup();
            col_gr4 = scene.physics.add.staticGroup();
            ...
            player = scene.physics.add.sprite(100, 450, 'sami').setScale(0.08);
            player.setCollideWorldBounds(true);


            scene.anims.create({
                key: 'left',
                frames: scene.anims.generateFrameNumbers('sami', { start: 1, end: 4 }),
                frameRate: 10,
                repeat: -1
            });
            ...
            scene.anims.create({
                key: 'back',
                frames: scene.anims.generateFrameNumbers('sami', { start: 5, end: 8 }),
                frameRate: 10,
                repeat: -1
            });
            scene.anims.create({
                key: 'front',
                frames: scene.anims.generateFrameNumbers('sami', { start: 9, end: 12 }),
                frameRate: 10,
                repeat: -1
            });
            ...
            scene.anims.create({
                key: 'right',
                frames: scene.anims.generateFrameNumbers('sami', { start: 13, end: 16  }),
                frameRate: 10,
                repeat: -1
            });
            ...
            //플레이어와 충돌
            scene.physics.add.collider(player, house_m);
            scene.physics.add.collider(player, trees);
            scene.physics.add.collider(player, col_left_h_g);
            scene.physics.add.collider(player, col_mid_h);
            scene.physics.add.collider(player, col_mini_h);
            scene.physics.add.collider(player, col_left_h);
            scene.physics.add.collider(player, col_right_h);
            scene.physics.add.collider(player, col_north_o);
            scene.physics.add.collider(player, col_west_o);
            scene.physics.add.collider(player, col_fo1);
            scene.physics.add.collider(player, col_fo2);
            scene.physics.add.collider(player, col_fo3);
            scene.physics.add.collider(player, col_fo4);
            scene.physics.add.collider(player, col_fo5);
            scene.physics.add.collider(player, col_fo6);
            scene.physics.add.collider(player, col_gr1);
            scene.physics.add.collider(player, col_gr2);
            scene.physics.add.collider(player, col_gr3);
            scene.physics.add.collider(player, col_gr4);
